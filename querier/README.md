Aya Hajjeh Lab6 22Fall 31 OCt, 2022

***NOTE for Lab6 functionality, I implemented the fourth subset described in the lab description on the course webpage as my querier supports `and` and `or` operators, with `or` precedence over `and`, and that prints the final query result in decreasing order by score.

***NOTE there is no consistency in the naming conventions of this lab as some names are camelCase and others are snake_case as the lab description, activity files, and functions prototypes provided from the website are not consistent in their style. I chose to keep the names in the same style as they were mentioned on the course webpage.
